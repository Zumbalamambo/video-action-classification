...
with tf.Session() as sess:
    ...
    # build fusion layer
    fc_spatial = fc_layer("fc_spatial", spatial_features, (512, 200), "relu", 0.0, 0.001)
    fc_motion = fc_layer("fc_motion", motion_features, (512, 200), "relu", 0.0, 0.001)

    with tf.variable_scope("fusion"):
        fusion_layer_input = tf.concat([fc_spatial, fc_motion], axis=1)
    fusion = fc_layer("fusion", fusion_layer_input, (400, 200), "relu", 0.0, 0.001)

    logits = fc_layer("softmax", fusion, (200, C), "linear", 0.0, 0.001)
    ...
    with tf.variable_scope("", reuse=True):
        fusion_weights = tf.get_variable("fusion/fusion_weights")
        fusion_biases = tf.get_variable("fusion/fusion_biases")
        p_grads = proximal_gradient_optimizer.compute_gradients(loss, var_list=[fusion_weights, fusion_biases])
